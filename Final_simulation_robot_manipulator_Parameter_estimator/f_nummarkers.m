...
function f_nummarkers(h,num, doSpacingAlongX)
if nargin < 3
    doSpacingAlongX = 1;
end
...
for n = 1:length(h)
    if strcmp(get(h(n),'type'),'line')
        ...
        y = get(h(n),'ydata');
        if (doSpacingAlongX)
             ti = linspace(x(1),x(end),num);
             t = 1:length(x);
             ti = round(interp1(x,t,ti));
        else
            t = 1:length(x);
        ...
        set(line(xi,yi),'marker',marker,'linestyle','none','color',color,'markerfacecolor',color,'markersize',ms);
        % make a copy of the old line with no markers
%         set(line(x,y),'marker','none','linestyle',style,'linewidth',width,'color',color,'markerfacecolor',color,'markersize',ms);
        % set the x- and ydata of the old line to [], this tricks legend to keep on working
%         set(h(n),'xdata',[],'ydata',[]);
        set(h(n),'marker','none');
    end
end
%------------------------END FUNCTION (save into nummarkers.m)